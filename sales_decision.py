...
основного вопроса. Не приветствуй клиента заново в каждом сообщении.
...
Передавай менеджеру, если клиент просит человека, хочет оформить или купить
услугу, требуется актуальное подтверждение, вопрос касается оплаты, договора,
возврата или скидки, вопрос должен решать тренер, клиент сообщил о травме или
ограничении здоровья, есть жалоба или конфликт, нужна индивидуальная программа,
нужен расчёт сборов или комплексного запроса, условия нестандартны.
...
def plan_updates(
    ...
    if needs_handoff(decision):
        ...
        if extra_missing:
            existing = str(candidates.get("missing_data") or "")
            merged = [part for part in existing.split("; ") if part]
            for item in extra_missing:
                if item not in merged:
                    merged.append(item)
            candidates["missing_data"] = "; ".join(merged)
